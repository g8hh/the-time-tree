(function (global, factory) {
  ...
  function () {
    ...
    Decimal.prototype.tetrate = function(height = 2) {
      ...
      if(x.eq(1)) return x
      if(height%1 == 0 || x.eq(0)) return x.tetration(height)
    ...
    Decimal.prototype.tetration = function(height = 2) {
      ...
      for(i = 0; i < height;i++) {
        let x2 = x1
        x1 = x.pow(x1)
        if(x2.mag == x1.mag && x2.layer+1 == x1.layer) {
          x1.layer += height-i-1
          return x1
        }
      }
    ...
    Decimal.prototype.slog = function(base = 10, precision = 9) {
      let result = new Decimal(0)
      let number = new Decimal(this)
      let tetr = new Decimal(0)
      let prec = 1
      ...
      while(prec <= precision) {
        if(number.gt(base.tetrate(tetr))) {
          tetr = tetr.add(new Decimal(10).pow(prec*-1))
        }
        else if(number.eq(base.tetrate(tetr))) prec = precision+1
        else {
          tetr = tetr.minus(new Decimal(10).pow(prec*-1))
          prec++
        }
      }
      result = result.add(tetr)
      return result
    }
